    ...
    def _display_analysis_results(self, scenarios: List[Dict]):
        ...
        for i, scenario in enumerate(scenarios):
            ...
            for tf, analysis in scenario["timeframes"].items():
                ...
                print("  Indicators:")
                print(f"    Price Momentum: {'YES' if analysis['price_momentum'] else 'NO'}")
                print(f"    RSI Momentum: {'YES' if analysis['rsi_momentum'] else 'NO'}")
                print(f"    Volume Spike: {'YES' if analysis['volume_spike'] else 'NO'}")
                print(f"    MA Cross: {'YES' if analysis['ma_cross'] else 'NO'}")
...
def compare_symbols():
    ...
    print(f"\n{'Symbol':<12} {'Timeframe':<12} {'Price Change%':<10} {'ATR%':<8} {'Threshold%':<12} {'Momentum':<10}")
    print("-" * 80)
...
def main():
    ...
    analyzer.run_comprehensive_analysis("BTCUSDC", periods=300)
    
    # Compare different symbols
    print("\n2. Comparing across symbols...")
    compare_symbols()
    
    # Demonstrate flexibility features
    print("\n3. Demonstrating system flexibility...")
