...
def suggest_alternative_trades(from_asset, to_asset, min_notional_needed):
    """Suggest alternative trading strategies when single-asset trade fails"""
    print(f"\n=== ALTERNATIVE TRADING SUGGESTIONS ===")
    
    # Refresh balances to get accurate data
    fetch_all_balances()
    ...
    total_value = btc_value + eth_value + balance['usd']
    
    print(f"Current Portfolio Values:")
    print(f"  BTC: {balance['btc']:.6f} (~${btc_value:.2f})")
    print(f"  ETH: {balance['eth']:.6f} (~${eth_value:.2f})")
    print(f"  USDC: ${balance['usd']:.2f}")
    ...
    if to_asset == 'USDC':
        # Want to convert to USDC
        if btc_value >= min_notional_needed and from_asset != 'BTC':
            suggestions.append(f"• Sell BTC instead: {balance['btc']:.6f} BTC → ${btc_value:.2f} USDC")
        if eth_value >= min_notional_needed and from_asset != 'ETH':
            suggestions.append(f"• Sell ETH instead: {balance['eth']:.6f} ETH → ${eth_value:.2f} USDC")
        
        # Multi-asset liquidation
        combined_value = (btc_value if from_asset != 'BTC' else 0) + (eth_value if from_asset != 'ETH' else 0)
        if combined_value >= min_notional_needed:
            suggestions.append(f"• Partial liquidation: Combine assets for ${combined_value:.2f} total")
            
    elif from_asset == 'USDC' and balance['usd'] < min_notional_needed:
        # Need more USDC
        if btc_value >= 5:  # Minimum viable conversion
            suggestions.append(f"• Convert BTC to USDC first: ${btc_value:.2f} available")
        if eth_value >= 5:
            suggestions.append(f"• Convert ETH to USDC first: ${eth_value:.2f} available")
    ...
    if suggestions:
        ...
        print(f"   • Add more funds to your account")
        print(f"   • Wait for price movements")
        print(f"   • Reduce position sizes")
    
    print(f"=== END SUGGESTIONS ===\n")
    ...
    try:
        ...
        if trade_value < min_notional:
            ...
            if balance[balance_key] >= adjusted_amount:
                ...
                # For SELL operations, try using maximum available balance if it's reasonably close
                if side == 'SELL':
                    max_available = balance[balance_key]
                    max_rounded = round_qty(symbol, max_available)
                    max_value = max_rounded * current_price
                    
                    # If max available is within 90% of minimum notional, allow the trade
                    if max_value >= (min_notional * 0.90) and max_rounded > 0:
                        print(f"[AUTO-ADJUST] Using maximum available balance: {max_rounded:.8f} (${max_value:.2f})")
                        print(f"[AUTO-ADJUST] Trade allowed as it's {(max_value/min_notional)*100:.1f}% of minimum notional")
                        rounded_amount = max_rounded
                        trade_value = max_value
                    else:
                        print(f"[SKIP] Insufficient balance for minimum notional trade:")
                        print(f"   Required: {adjusted_amount:.8f} {from_asset} (${min_notional:.2f})")
                        print(f"   Available: {max_available:.8f} {from_asset} (${max_value:.2f})")
                        
                        # Provide alternative trading suggestions
                        suggest_alternative_trades(from_asset, to_asset, min_notional)
                        return None
                else:
                    # For BUY operations, we need exact minimum notional
